...
#        print ("open serial port: %s" % sys.argv[1])
        ser = Serial(comNo, 115200, timeout=0.1)
        print ("open serial port: %s" % comNo)
    except:
        print ("cannot open serial port: %s" % comNo)
        exit(1)
...
def SendMessage(comNo, message):
    try:
        ser = Serial(comNo, 115200, timeout=0.1)
        cmd = message + "\r\n"
        print ("open serial port: %s" % comNo)
        print ("--> "+ str(cmd.encode('utf-8')))
        ser.write(cmd.encode('utf-8'))
    except:
        print ("cannot open serial port: %s" % comNo)
        exit(1)
